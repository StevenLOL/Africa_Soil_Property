...
%training(:,3579)=(training(:,3579)+1)/2;
%sortedtest(:,3579)=(sortedtest(:,3579)+1)/2;
...
for loopSmooth=1:101
    ...
    if hasgpu==1
        ...
        test_y=full_test_y;
        nn = nnsetup([3578 layersize layersize 5]);      
        ...
        eva1 = nnoutput( nn, eval_x );
        RMSE =mean( sqrt( sum( (test_y-eva1).^2) / numel(test_y) ));
    ...
    avgRMSE{loopSmooth}=RMSE;
    
    if sum([1,5,20,35,50,100]==loopSmooth)>0

        avgToWrite=mean(reshape(cell2mat(avgPrediction), [727, 5, loopSmooth]), 3);
        avgRMSEFileName=mean(mean(reshape(cell2mat(avgRMSE), [1, 1, loopSmooth]), 3));
        filename=sprintf('dnn5t_%d_%d_mean%f.csv',layersize,loopSmooth,avgRMSEFileName);
